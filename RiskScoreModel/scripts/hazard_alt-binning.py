...
for month in tqdm(hazard_df.timeperiod.unique()):
    ...
    hazard_df_month['drainage_density_binned'] = log_quantile_binning(hazard_df_month, 'drainage_density')
    hazard_df_month['mean_rain_binned'] = log_quantile_binning(hazard_df_month, 'mean_rain')
    hazard_df_month['max_rain_binned'] = log_quantile_binning(hazard_df_month, 'max_rain')
    hazard_df_month['Sum_Runoff_binned'] = log_quantile_binning(hazard_df_month, 'Sum_Runoff')
    hazard_df_month['Peak_Runoff_binned'] = log_quantile_binning(hazard_df_month, 'Peak_Runoff')
    hazard_df_month['slope_mean_binned'] = log_quantile_binning(hazard_df_month, 'slope_mean')
    hazard_df_month['elevation_mean_binned'] = custom_binning_reversed(hazard_df_month, 'elevation_mean')
    hazard_df_month['distance_from_river_mean_binned'] = custom_binning_reversed(hazard_df_month, 'distance_from_river_mean')
    
    # Average hazard score
    hazard_df_month['flood-hazard'] = (hazard_df_month[['drainage_density_binned', 'mean_rain_binned', 
...
master_variables.to_csv(path + r'/RiskScoreModel/data/hazard_logquantile-bin_2.csv', index=False)